...
export class ScheduleHandler extends BaseApiPrismaHandler {
  ...
  override async post(req: Request, res: Response): Promise<void> {
    const schedulesRequests: Omit<ScheduleRequest, "id">[] =
      req.body.schedules ?? [];
  ...
  override async get(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (req.query.id) {
        ...
        // if no id is supplied, return all schedules
        scheduleResuls = await this.prismaClient.schedule.findMany({
          include: { tasks: true },
        });
      }
  ...
  override async put(req: Request, res: Response): Promise<void> {
    ...
    if (!scheduleId || !scheduleRequest) {
      res
        .status(400)
        .send({
          msg: "Bad request: no schedule id or schedule details provided to perform update.",
        });
      return;
    }
    ...
    try {
      ...
      if (e instanceof Prisma.PrismaClientKnownRequestError) {
        res
          .status(404)
          .send({ msg: "Bad Request: schedule id does not exist in DB." });
      } else {
  ...
  override async delete(req: Request, res: Response): Promise<void> {
    ...
    if (!scheduleId) {
      res
        .status(400)
        .send({
          msg: "Bad request: no schedule id provided to perform delete.",
        });
      return;
    }
